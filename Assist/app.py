...
@app.route('/api/assistants/<int:assistant_id>/messages', methods=['GET'])
def get_assistant_messages(assistant_id):
    try:
        # Check if assistant exists
        assistant_data = db.db_get_assistant(assistant_id)
        if not assistant_data:
            return jsonify({"error": f"Assistant with ID {assistant_id} not found"}), 404

        # Get all messages for this assistant from all chats
        messages_query = """
        SELECT
            c.openai_thread_id as thread_id,
            m.assistant_id,
            m.openai_message_id as message_id,
            m.message_text as text,
            m.created_at,
            m.updated_at
        FROM messages m
        JOIN chats c ON m.chat_id = c.id
        WHERE m.assistant_id = %s
        ORDER BY m.created_at DESC
        """

        # Execute query using database connection with psycopg3
        import psycopg
        ...
        with conn.cursor() as cursor:
            cursor.execute(messages_query, (assistant_id,))
        ...
        app.logger.error(f"Error getting assistant messages: {e}", exc_info=True)
